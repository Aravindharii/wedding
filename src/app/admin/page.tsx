...
export default function Dashboard() {
  ...
  useEffect(() => {
    getDocs(collection(db, "guests")).then(snap =>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        {cards.map((c, i) => (
          <motion.div key={c.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            className={`bg-surface/80 backdrop-blur rounded-2xl p-6 border border-ink/10 shadow-[0_18px_60px_rgba(15,18,34,0.10)] border-l-4 ${c.color}`}>
